...
export const pagesPath = {
	"posts": {
		_id: (id: string | number) => ({
			$url: (url?: { hash?: string }) => ({ pathname: "/posts/[id]" as const, query: { id }, hash: url?.hash, path: `/posts/${id}${buildSuffix(url)}` })
		}),
		"new": {
			$url: (url?: { hash?: string }) => ({ pathname: "/posts/new" as const, hash: url?.hash, path: `/posts/new${buildSuffix(url)}` })
		}
	},
	"signin": {
